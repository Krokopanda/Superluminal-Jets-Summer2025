...
def custom_dist_loglike(data, c, sigma, x):
    ...
    return loglikewithgrad_op(c, sigma, x, data)
...
class LogLikeWithGrad(Op):
    ...
    def grad(
        ...
        grad_wrt_c = loglikegrad_op(c, sigma, x, data)
...
class LogLikeGrad(Op):
    def make_node(self, c, sigma, x, data) -> Apply:
        ...
        # TODO double check this if errors
    ...
    def perform(
        ...
        # calculate gradients
        grad_wrt_c = finite_differences_loglike(c, sigma, x, data)

        outputs[0][0] = grad_wrt_c


def finite_differences_loglike(c, sigma, x, data, eps=1e-7):
    """
    ...
    def inner_func(c, sigma, x, data):
        return my_loglike(*c, sigma, x, data)

    grad_wrt_mc = approx_fprime([c], inner_func, [eps, eps], sigma, x, data)

    return grad_wrt_mc[:, 0]
...
c = pt.scalar("c")
out = loglikegrad_op(c, sigma, x, data)
# set up our data
...
finite_differences_loglike(ctrue, sigma, x, data)
# Initialize the Ops
